import { Job } from "@/lib/types";

export const privateJobsSeed: Job[] = [
  {
    id: "reply-frontend-developer",
    title: "Frontend Developer",
    company: "Reply",
    sector: "privato",
    organizationGovernance: "private",
    organizationDomain: "company-careers",
    location: "Torino, Piemonte, Italia",
    city: "Torino",
    workMode: "hybrid",
    contractType: "indeterminato",
    source: "Company Careers",
    sourceType: "company-site",
    originalUrl: "https://www.reply.com/it/careers/job-details/frontend-developer-torino",
    postedAt: "2026-03-20",
    expiresAt: null,
    discoveredAt: "2026-03-23",
    tags: ["react", "typescript", "frontend", "next.js"],
    summary: "Ruolo frontend con stack React e TypeScript, presenza in sede 2 giorni a settimana.",
    status: "nuova"
  },
  {
    id: "intesa-data-analyst",
    title: "Data Analyst",
    company: "Intesa Sanpaolo",
    sector: "privato",
    organizationGovernance: "private",
    organizationDomain: "finance",
    location: "Torino, Piemonte, Italia",
    city: "Torino",
    workMode: "hybrid",
    contractType: "indeterminato",
    source: "Company Careers",
    sourceType: "company-site",
    originalUrl: "https://group.intesasanpaolo.com/it/careers/posizioni-aperta/data-analyst-torino",
    postedAt: "2026-03-18",
    expiresAt: null,
    discoveredAt: "2026-03-23",
    tags: ["sql", "python", "data", "analytics"],
    summary: "Analisi dati e reporting su processi interni, team basato su Torino.",
    status: "vista"
  },
  {
    id: "prima-software-engineer",
    title: "Software Engineer",
    company: "Prima Industrie",
    sector: "privato",
    organizationGovernance: "private",
    organizationDomain: "company-careers",
    location: "Collegno, Torino, Italia",
    city: "Collegno",
    workMode: "on-site",
    contractType: "indeterminato",
    source: "Company Careers",
    sourceType: "company-site",
    originalUrl: "https://www.primaindustrie.com/careers/software-engineer-collegno",
    postedAt: "2026-03-19",
    expiresAt: null,
    discoveredAt: "2026-03-23",
    tags: ["c#", "automation", "industry", ".net"],
    summary: "Sviluppo software per sistemi industriali con sede nell'area torinese.",
    status: "salvata"
  },
  {
    id: "csi-pmo",
    title: "Project Management Officer",
    company: "CSI Piemonte",
    sector: "pubblico",
    organizationGovernance: "hybrid",
    organizationDomain: "ict",
    organizationNatureNote:
      "Classificazione operativa: ente a governance ibrida; verifica sempre natura dell'ente e regime della selezione sul sito ufficiale.",
    location: "Torino, Piemonte, Italia",
    city: "Torino",
    workMode: "hybrid",
    contractType: "determinato",
    source: "Organization Jobs",
    sourceType: "company-site",
    originalUrl: "https://www.csipiemonte.it/lavora-con-noi/project-management-officer-2026",
    postedAt: "2026-03-15",
    expiresAt: null,
    discoveredAt: "2026-03-23",
    tags: ["project-management", "pmo", "digitale"],
    summary: "Supporto a progetti digitali in un'organizzazione ICT con base a Torino.",
    status: "nuova"
  },
  {
    id: "sorint-react-engineer",
    title: "React Engineer",
    company: "Sorint.Lab",
    sector: "privato",
    organizationGovernance: "private",
    organizationDomain: "ict",
    location: "Milano, Lombardia, Italia",
    city: "Milano",
    workMode: "hybrid",
    contractType: "indeterminato",
    source: "Company Careers",
    sourceType: "company-site",
    originalUrl: "https://careers.sorint.com/react-engineer-milano",
    postedAt: "2026-03-22",
    expiresAt: null,
    discoveredAt: "2026-03-23",
    tags: ["react", "typescript", "frontend", "agile"],
    summary: "Ruolo frontend per prodotti enterprise con stack React e TypeScript, sede Milano.",
    status: "nuova"
  },
  {
    id: "engineering-java-analyst",
    title: "Java Analyst Developer",
    company: "Engineering",
    sector: "privato",
    organizationGovernance: "private",
    organizationDomain: "ict",
    location: "Roma, Lazio, Italia",
    city: "Roma",
    workMode: "hybrid",
    contractType: "indeterminato",
    source: "Company Careers",
    sourceType: "company-site",
    originalUrl: "https://www.eng.it/careers/java-analyst-developer-roma",
    postedAt: "2026-03-17",
    expiresAt: null,
    discoveredAt: "2026-03-23",
    tags: ["java", "spring", "backend", "analysis"],
    summary: "Analisi e sviluppo Java Spring per clienti enterprise con team basato a Roma.",
    status: "nuova"
  },
  {
    id: "hera-data-engineer-bologna",
    title: "Data Engineer",
    company: "Gruppo Hera",
    sector: "privato",
    organizationGovernance: "private",
    organizationDomain: "company-careers",
    location: "Bologna, Emilia-Romagna, Italia",
    city: "Bologna",
    workMode: "hybrid",
    contractType: "determinato",
    source: "Company Careers",
    sourceType: "company-site",
    originalUrl: "https://www.gruppohera.it/careers/data-engineer-bologna",
    postedAt: "2026-03-16",
    expiresAt: null,
    discoveredAt: "2026-03-23",
    tags: ["data", "python", "sql", "etl"],
    summary: "Pipeline dati e analytics per team innovation con presenza su Bologna.",
    status: "nuova"
  },
  {
    id: "alten-backend-remote",
    title: "Backend Developer",
    company: "Alten Italia",
    sector: "privato",
    organizationGovernance: "private",
    organizationDomain: "ict",
    location: "Torino, Piemonte, Italia",
    city: "Torino",
    workMode: "remote",
    contractType: "indeterminato",
    source: "Company Careers",
    sourceType: "company-site",
    originalUrl: "https://www.alten.it/carriere/backend-developer-torino",
    postedAt: "2026-03-21",
    expiresAt: null,
    discoveredAt: "2026-03-23",
    tags: ["java", "spring", "backend"],
    summary: "Ruolo fully remote, mantenuto solo per testare l'esclusione automatica.",
    status: "nuova"
  }
];
